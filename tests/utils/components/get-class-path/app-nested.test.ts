...
test('utils | components | get-class-path > app (nested)', function () {
  assert.strictEqual(
    getPath('index'),
    normalizeFilePath('app/components/index/index.ts'),
  );

  assert.strictEqual(
    getPath('navigation-menu'),
    normalizeFilePath('app/components/navigation-menu/index.ts'),
  );

  assert.strictEqual(
    getPath('widgets/widget-3'),
    normalizeFilePath('app/components/widgets/widget-3/index.ts'),
  );
});
